...
- `\infer[Rule]{...}{...}` の `Rule` は proof.sty と同じく数式モードの内容として扱い、`\RightLabel{$Rule$}` の形で出力します。
- 前提は、最上位の `&` だけで分割します。
...
- The `Rule` part of `\infer[Rule]{...}{...}` is treated as math content, as in proof.sty, and is emitted as `\RightLabel{$Rule$}`.
